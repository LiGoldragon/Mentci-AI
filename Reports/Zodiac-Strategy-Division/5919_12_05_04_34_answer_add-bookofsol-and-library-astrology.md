...
## Prompt
implement a minimal Library/astrology that contains references for correspondance and symbolism of astrology. use the ligoldragon/bookofsol from github for reference (add it to Inputs - add in flake and mount to read)

## Outcome
Implemented minimal astrology reference layer and wired `bookofsol` as an input mounted into `Inputs/`.

### Input wiring
- `flake.nix`
  - added input: `bookofsol = { url = "github:LiGoldragon/bookofsol"; flake = false; }`
- `Components/nix/jail_inputs.nix`
...
Both reference `Inputs/bookofsol` as source and provide minimal symbolic orientation.
