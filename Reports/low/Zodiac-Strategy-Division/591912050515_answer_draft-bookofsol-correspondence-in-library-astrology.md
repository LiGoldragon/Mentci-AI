...
- `Sources/bookofsol` is currently not mounted/present in this workspace.
- Flake resolution for `bookofsol` is currently unavailable in this environment.
- The new draft is marked provisional and includes a reconciliation TODO for exact source citations once `Sources/bookofsol` is available.
