...
3. Run `git filter-repo` with inverted paths for:
- `workspace`
- `$(pwd)`
4. Clean stale refs/reflogs and run aggressive gc.
...
1. Import rewritten refs in JJ and resolve bookmark targets.
2. Confirm `dev` and `main` point to intended post-rewrite commits.
3. Force-push rewritten `dev` and `main`.
