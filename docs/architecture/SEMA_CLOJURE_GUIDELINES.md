...
3. **Schema Is Sema**
   Schemas are the authoritative truth. Encodings are incidental. A Sema object always
   contains a `:sema/type` field.

## Clojure Object Model

### Object Definition
Sema objects are EDN maps with a mandatory `:sema/type` and a Malli schema.
...
(def GreetInput
  [:map
   [:sema/type [:= "GreetInput"]]
   [:name :string]])

(def GreetOutput
  [:map
   [:sema/type [:= "GreetOutput"]]
   [:message :string]])
...
*   All schemas must include `:sema/type` with an exact string literal.
