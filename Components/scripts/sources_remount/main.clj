...
(defprotocol SourcesRemountOps
  ...
  (remount-source-for [this input])
  (strip-write-permissions-for [this input]))

(defrecord DefaultSourcesRemount [])

(impl DefaultSourcesRemount SourcesRemountOps fail-for FailInput :any
  [this input]
  (binding [*out* *err*]
    (println (:message input)))
  (System/exit 1))

(impl DefaultSourcesRemount SourcesRemountOps parse-args-for ParseArgsInput RemountArgs
  [this input]
  (loop [remaining (:args input)
         options {:attrsPath ".attrs.json"
                  :sourcesPath nil
                  :whitelistPath "Core/agent-sources.edn"}]
    (if (empty? remaining)
      options
      (let [[arg & _more] remaining]
        (cond
          ...
          (= arg "--sources-path")
          (recur (nnext remaining) (assoc options :sourcesPath (second remaining)))
          ...
          (= arg "--help")
          (do
            (println "Usage: bb Components/scripts/sources_remount/main.clj [--attrs-path <path>] [--sources-path <path>] [--whitelist-path <path>]")
            (System/exit 0))

          :else
          (fail-for this {:message (str "Unknown argument: " arg)}))))))

(impl DefaultSourcesRemount SourcesRemountOps keywordize-keys-for KeywordizeKeysInput :map
  [this input]
  (let [data (:data input)]
    (into {}
          (for [[k v] data]
...
(impl DefaultSourcesRemount SourcesRemountOps find-jail-config-for FindJailConfigInput [:maybe :map]
  [this input]
  (let [data (:data input)]
    (cond
...
(impl DefaultSourcesRemount SourcesRemountOps read-jail-config-for ReadConfigInput types/JailConfig
  [this input]
...
(impl DefaultSourcesRemount SourcesRemountOps read-whitelist-for WhitelistInput [:set :string]
  [this input]
...
(impl DefaultSourcesRemount SourcesRemountOps delete-path-for DeletePathInput :any
  [this input]
...
(impl DefaultSourcesRemount SourcesRemountOps remount-source-for RemountInput :any
  [this input]
...
(impl DefaultSourcesRemount SourcesRemountOps strip-write-permissions-for StripWriteInput :map
  [this input]
...
(def default-sources-remount (->DefaultSourcesRemount))

(main Input
  [input]
  (let [args (parse-args-for default-sources-remount {:args (:args input)})
        config (read-jail-config-for default-sources-remount {:attrsPath (:attrsPath args)})
        whitelist (read-whitelist-for default-sources-remount {:whitelistPath (:whitelistPath args)})
        sources-path (or (:sourcesPath args) (:sourcesPath config) "Sources")
        source-manifest (:sourceManifest config)
        sources-root (io/file sources-path)]
    (.mkdirs sources-root)
    (println (str "Refreshing Sources mount at: " (.getPath sources-root)))
      ...
      (let [name-str (name item-name)]
        (when (contains? whitelist name-str)
          (let [source-path (or (:srcPath item-spec) (:sourcePath item-spec))
                target-path (.getPath (io/file sources-root name-str))]
            (println (str "Re-mounting " name-str " -> " source-path))
            (remount-source-for default-sources-remount {:name name-str
                                                       :sourcePath source-path
                                                       :targetPath target-path})
            (let [report (strip-write-permissions-for default-sources-remount {:path source-path})]
              (println (format "RO enforced for %s source tree (visited=%d changed=%d skipped=%d)"
                               name-str
                               (:visited report)
                               (:changed report)
                               (:skipped report))))))))
    (println "Sources refresh complete.")))
