...
(defrecord DefaultSourcesMounter [])

(impl DefaultSourcesMounter SourcesMounterOps fail-for {:message :string} :any
  [this input]
  (binding [*out* *err*]
    (println (:message input)))
  (System/exit 1))

(impl DefaultSourcesMounter SourcesMounterOps parse-args-for ParseArgsInput :map
  [this input]
    ...
    (if (empty? remaining)
      ...
      (let [[arg & _] remaining]
        (cond
          ...
          (= arg "--sources-root")
          (recur (nnext remaining) (assoc opts :sourcesRoot (second remaining)))
          ...
          (= arg "--help")
          (do
            (println "Usage: bb Components/scripts/sources_mounter/main.clj [--attrs .attrs.json] [--sources-root Sources] [--mode ro|rw] [--whitelist Core/agent-sources.edn] [--replace] [--write]")
            (System/exit 0))

          :else
          (fail-for this {:message (str "Unknown argument: " arg)}))))))

(impl DefaultSourcesMounter SourcesMounterOps keywordize-keys-for KeywordizeKeysInput :map
  [this input]
  (let [data (:data input)]
    (into {}
          (for [[k v] data]
...
(impl DefaultSourcesMounter SourcesMounterOps find-jail-config-for FindJailConfigInput [:maybe :map]
  [this input]
  (let [data (:data input)]
    (cond
      (and (map? data) (get data "sourceManifest")) data
      (and (map? data) (get data :sourceManifest)) data
...
(impl DefaultSourcesMounter SourcesMounterOps load-whitelist-for LoadWhitelistInput [:set :string]
  [this input]
...
(impl DefaultSourcesMounter SourcesMounterOps key-name-for {:value :any} :string
  [this input]
  (let [v (:value input)]
    (cond
      (keyword? v) (name v)
...
(impl DefaultSourcesMounter SourcesMounterOps ensure-parent-for {:path :string} :any
  [this input]
  (io/make-parents (str (:path input) "/.keep")))

(impl DefaultSourcesMounter SourcesMounterOps ensure-link-for EnsureLinkInput :map
  [this input]
...
(impl DefaultSourcesMounter SourcesMounterOps run-mounter-for Input :any
  [this input]
  (let [{:keys [attrs sourcesRoot mode replace? write? whitelistPath]} (parse-args-for this {:args (:args input)})
    ...
    (let [raw (json/parse-string (slurp attrs-file))
          ...
          manifest (:sourceManifest config)
          _ (ensure-parent-for this {:path (str sourcesRoot "/.keep")})
          results (->> manifest
                       (sort-by key)
                                 ...
                                 (let [name-str (key-name-for this {:value name})]
                                   (if (contains? whitelist name-str)
                                     (let [source (or (:srcPath spec) (:sourcePath spec))
                                           target (str sourcesRoot "/" name-str)
                                     ...
                                     (conj acc {:action "skip-not-whitelisted"
                                                ...
                                                :source nil}))))
                               []))]
      (println (str "Sources mounter mode: " mode))
      (println (str "Attrs source: " attrs))
      (println (str "Sources root: " sourcesRoot))
      (doseq [r results]
...
(def default-sources-mounter (->DefaultSourcesMounter))

(main Input
  [input]
  (run-mounter-for default-sources-mounter input))
