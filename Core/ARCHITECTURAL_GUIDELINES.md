...
*   **Cap'n Proto Prebuilt Messages:** When passing structured data to a component, use a prebuilt Cap'n Proto binary message stored alongside a text-version (e.g., JSON or EDN) in the component's data directory. Use a hash of the text-version to keep them synced, either by embedding the hash in the filename (e.g., `config_<hash>.bin`) or inside the payload, and verify this synchronization automatically. **DO NOT write ad-hoc bash `sync.sh` scripts for this.** Use or extend the `mentci-mcp` tools (e.g., `capnp_sync_protocol`) to handle this algorithmically.
...
*   **SEMA:** The machine-code symbolic language underpinning all technology. (Deprecated: *Sajban* when referring to binary code).
*   **Aski:** The text-native representation layer.
*   **Lojix (aski-lojix):** The technical Aski variant.
*   **Sajban (aski-sajban):** The natural-language Aski variant. It is self-loading/rendering from SEMA binary.
