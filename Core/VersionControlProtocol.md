...
2.1. **USE GIT THROUGH JJ IN JJ REPOS:** In any JJ repo, including nested component repos, we only use Git through JJ. Do not use `git status`, `git log`, `git branch`, `git checkout`, `git commit`, `git merge`, or `git push` as authoritative workflow operations when `jj` can answer the question. If a mistaken Git action already occurred, treat it as imported backend state to recover from with JJ; do not continue the workflow in Git.
2.2. **NON-INTERACTIVE JJ AUTOMATION RULE:** Agent and subagent JJ commands must run in explicit no-editor mode. Set `JJ_EDITOR=:` (and, when needed, `VISUAL=:` / `EDITOR=:`) for automated shells, and use explicit message/target flags instead of any command shape that would invoke an editor. Use `-m`/`--message` for `jj commit` and `jj describe`, explicit rev/target flags for `jj new`, and `--into` for `jj squash`. If a JJ command would open an editor, reformulate it before execution.
\n+2.2.1. **NO-EDITOR INVOCATION ENFORCEMENT:** All automation that runs JJ from a shell (scripts, agents, subagents, or manual automation) must prefix commands with `env JJ_EDITOR=: VISUAL=: EDITOR=:` and never rely on interactive editors. This ensures no-editor automation is enforced at the protocol level and avoids accidental blocking prompts during preflight, commits, bookmark moves, or pushes.
...
11.1. **NO DANGLING VISIBLE-COMMIT EXIT:** Do not end a prompt with extra visible heads, described empty commits, or side histories left dangling unless they are intentionally preserved and explicitly classified. If any visible non-target revision remains after the main finalize/push flow, classify it as active, integrated, intentionally preserved, or cleanup candidate, record why it remains, and clean it before completion if it is mere noise.
