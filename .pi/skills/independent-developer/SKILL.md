...
- **Dirty-Worktree Handoff Rule:** A non-clean JJ working copy must be treated as a handoff state, whether intentional or accidental (disconnect, crash, blocked task, human interruption, etc.). If the session leaves behind useful local context that is not ready to commit, the agent SHOULD write a plainly named restart-context artifact in the active workspace root so the next session can recover intent quickly. Prefer leaving such handoff artifacts uncommitted unless the user explicitly asks to publish them. A dirty tree is not automatically failure debris; it may be the only surviving handover packet.
...
- **No Pre-Emptive Describe Rule:** Keep the working-copy node anonymous while work is still in progress. Prefer `jj commit -m` on the intended non-empty revision; do not pre-emptively `jj describe` `@` or a working-copy wrapper just to make later bookmark movement or push feel easier.
- **Nested JJ Repo Rule:** If a component directory is itself a JJ repo (for example `Components/CriomOS`), treat it as a separate JJ context. We only use Git through JJ, so a visible `.git` directory there does not authorize direct Git commits, branch workflows, or Git-first state decisions.
