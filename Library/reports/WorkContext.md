...
1.  **Sources**: Incorporate `criomos` and `webpublish` (GitHub) as flake Sources.
2.  **Level 5 Tooling**: Provide tools for machine-level programming (GDB, strace, valgrind, and high-authority orchestration utils).
3.  **Jail Environment**: Implement a "pure flake jail" where the agent can operate with all Sources available in a writable workspace.

## Current State
- [x] **Repository Initialized**: `Mentci-AI` created with `jj` (git backend).
- [x] **Scaffolding**: `flake.nix`, `Components/mentci-aid/Cargo.toml`, and `Components/mentci-aid/src/main.rs` established.
- [x] **Locking**: `flake.lock` and `Components/mentci-aid/Cargo.lock` managed.
- [x] **Sources Integration**: `criomos`, `webpublish`, `sema`, `attractor`, and `opencode` integrated as flake Sources.
...
2.  [x] **Engine Expansion**: Implement the `Codergen` and `Wait.human` handlers in the Rust daemon (`Components/mentci-aid/src/main.rs`).

3.  [x] **Migration Path**: Execute the transition from Gemini CLI to the OpenCode/DeepSeek-V4 stack as per `Library/guides/MigrationGeminiToOpencode.md`. (Infrastructure established).

4.  [x] **Graph Syntax**: Implement DOT parser (`Components/mentci-aid/src/dot_loader.rs`) for defining agent workflows (`Components/workflows/*.dot`).
...
7.  [x] **Attractor Specs**: Extracted DOT data schema from `Sources/untyped/attractor` into `Library/specs/AttractorDotReference.md`.
