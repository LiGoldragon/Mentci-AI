...
### Pi Pre-Hooks and Post-Hooks (Active Integration Architecture)
To automate context gathering and auditability, Mentci-AI uses specialized prompt hooks and execute-based finalization utilities within the Pi environment:
...
### Reliability & Evidence Contract (Operational)
- Subagent completion reports must be non-empty and include at least a minimal status line.
- Claims about pushes/tests/builds must include raw command evidence snippets.
- On subagent failure (`Unknown agent`, missing model key, tooling mismatch), stop chain execution and report blocked state with exact error text.
