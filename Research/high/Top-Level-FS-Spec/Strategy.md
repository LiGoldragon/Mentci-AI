...
- Canonical-present: `Core`, `Library`, `Research`
...
`Library/specs/AskiFsSpec.md` defines `Components` as a canonical root in the enum, but operational text still references `src/` and `scripts/` directly as component paths. Strategy must first establish whether:
...
- `Research`: prompt/session answer artifacts grouped by subject.
- `Development`: pre-implementation planning and research per subject.
...
1. Specification
- Extend `Library/specs/AskiFsSpec.md` with enum-root model and sugared syntax mapping.
...
- Map `development/` -> `Development/`, `Core/` -> `Core/` until migration cutover.
...
- `development/` -> `Development/`
- update all references in `Core/`, `Library/`, `Research/`, scripts.
...
- create `Development/` and `Outputs/` (if absent at cutover)
...
- Record migration completion in Research and Restart Context.
