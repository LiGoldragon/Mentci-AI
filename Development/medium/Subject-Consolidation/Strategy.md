...
2. ≥50% file-reference overlap in research/strategy docs.
...
2. Update canonical `Research/<priority>/<Subject>/index.edn` entries.
...
3. `execute unify --write` keeps state stable (no oscillation).
4. Queue entries in `Library/StrategyQueue.md` point only to canonical subjects.
