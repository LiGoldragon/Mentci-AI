...
The same zodiac category lattice must be applied to both `Development/` and `Research/`
...
## Filesystem Shape
- `Development/<Zodiac-Category>/<Subject>/...`
- `Research/<Zodiac-Category>/<Subject>/...`
...
- Add lookup fallback for old `Development/<Subject>` paths where scripts assume flat layout.
- Add lookup fallback for old `Research/<Subject>` paths where scripts assume flat layout.
...
- When an agent opens `Development/<Zodiac-Category>/<Subject>/...`, it must also open
  `Research/<Zodiac-Category>/<Subject>/index.edn` (or latest report entry) before
...
- Tooling assumptions about `Development/<Subject>` flat layout.
...
## Acceptance Criteria
- Exactly 12 zodiac category directories exist under `Development/`.
- Exactly 12 zodiac category directories exist under `Research/`.
