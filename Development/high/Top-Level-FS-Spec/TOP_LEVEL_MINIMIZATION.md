...
1. The canonical 7 typed directories: `Sources/`, `Components/`, `Outputs/`, `Research/`, `Development/`, `Core/`, `Library/`.
...
- Session/answer artifacts -> `Research/`
- Planning artifacts -> `Development/`
...
## Guard Specification
Root check fails if:
- Any top-level directory is not in `{Sources, Components, Outputs, Research, Development, Core, Library}` or runtime exception set.
- Any top-level file is not in the required-file set or explicit conditional allowlist.
- Any lowercase duplicate of canonical roots appears (`core/`, `development/`, `sources/`, etc.).
