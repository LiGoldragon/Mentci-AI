...
- Shared sub-goal:
- Normalize runbook paths to `Sources/` and canonical goal references to `Core/HIGH_LEVEL_GOALS.md`.
- Combined deliverable:
...
- Shared sub-goal:
- Ensure UI/review surfaces anchor to canonical files (`Core/AGENTS.md`, `Core/HIGH_LEVEL_GOALS.md`) and `Sources/`.
- Combined deliverable:
...
2. Normalize repository path references to `Sources/` where path reality requires it.
